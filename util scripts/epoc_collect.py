...
def data_collection(welcome,index_name,filename,index,e):
    print(welcome)
    print '\n'
    old = filename
    while 1:
        try:
            print("This is the %d trial \n" % index )
            if os.path.isfile(filename):
                print('Boss. File already exists. Some mistake in indexing. To prevent overwriting. I am shutting down.')
                sys.exit(0)
                ...
                time.sleep(1)
                print("Starting.\n")
                eeg_data = e.acquire_data(7)
                print("Data acquired.\n")
                with open(filename,'wb') as eeg:
                    pickle.dump(eeg_data,eeg)
                ...
                print("Starting next trial. Waiting for 5 seconds \n")
                time.sleep(5)
                filename = ''.join([i for i in filename if not i.isdigit()])
                filename = filename.split('.')[0] + str(index) + '.pickle'                
...
def generate_args(welcome,index_name,filename):
    with open(index_name) as f:
        index = pickle.load(f)
    filename = filename + str(index) + '.pickle'
    return welcome,index_name,filename,index

def main():
    """Test function for EPOC class."""
    e = EPOC()
    print("Srinath, let's collect training data.")
    print("Choose among Clench and Eye")
    option = raw_input("Eye-1 Clench - 2 = ")
    index  = 11

    if(int(option) == 1):
        print("\n Choose among Blink,Open and Close-1,2,3")
        op = raw_input("Option 1 or 2 or 3 = ")

        if(int(op) == 1):
            welcome = "Welcome to Blink Training session."
            index_name = 'Eye/Blink/index.pickle'
            with open(index_name) as f:
                index = pickle.load(f)
            filename = 'Eye/Blink/EEG-Blinking' + str(index) + '.pickle'
            data_collection(welcome,index_name,filename,index,e)

        elif(int(op) == 2):
            welcome = "Welcome to Open Training session."
            index_name = 'Eye/Open/index.pickle'
            with open(index_name) as f:
                index = pickle.load(f)
            filename = 'Eye/Open/EEG-Opening' + str(index) + '.pickle'
            data_collection(welcome,index_name,filename,index,e)

        elif(int(op) == 3):
            welcome = "Welcome to Close Training session."
            index_name = 'Eye/Close/index.pickle'
            with open(index_name) as f:
                index = pickle.load(f)
            filename = 'Eye/Close/EEG-Closing' + str(index) + '.pickle'
            data_collection(welcome,index_name,filename,index,e)
        ...
        print("\n Choose among Clenching and Not-Clenching (1 or 2)")
        op = raw_input("Option 1 or 2 = ")
        if(int(op) == 1):
            welcome = "Welcome to Clench training session."
            index_name = 'Clench/Clench/index.pickle'
            with open(index_name) as f:
                index = pickle.load(f)
            filename = 'Clench/Clench/EEG-Clenching' + str(index) + '.pickle'
            data_collection(welcome,index_name,filename,index,e)

        elif(int(op) == 2):
            welcome = "Welcome to NON Clench training session."
            index_name = 'Clench/Non-Clench/index.pickle'
            with open(index_name) as f:
                index = pickle.load(f)
            filename = 'Clench/Non-Clench/EEG-NonClenching' + str(index) + '.pickle'
            data_collection(welcome,index_name,filename,index,e)
