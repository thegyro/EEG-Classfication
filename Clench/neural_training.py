...
class EegNeuralNetworkFeedForward:
    ...
    def populate_training_data(self,len_test):
        self.alldata = SupervisedDataSet(3,2)

        for klass in range(len(self.classes)):
            for fvector in self.feature_matrix[self.classes[klass]][:self.len_train-len_test]:
                if(klass == 0):
                    self.alldata.addSample(fvector,[1,0])
                else:
                    self.alldata.addSample(fvector,[0,1])

        #self.testdata,self.traindata = alldata.splitWithProportion(0.25)
    ...
    def train_the_network(self,no_hidden1,no_hidden2):
        ...
        self.trainer = BackpropTrainer(self.fnet,self.alldata)
        for i in range(125):
            self.trainer.train()
...
def main():
    ...
    eeg_neural = EegNeuralNetworkFeedForward(['Clench','Non-Clench'],128,channels,'/home/srinath/Code/PS-1/Code/Clench',7)
